...
export interface AutomationState {
   ...
   * /v1/automations/:id/run (v1-architecture.md §5.1, P0-4). The resident
   * agent runs the automation exactly once when this advances beyond its
   * last-consumed in-memory value. 0 = never triggered. Kept in sync with the
   * contract module's AutomationState so there is no third silently-different
   * shape. */
