...
final class AppModel {
    private static let credentialSchemaVersion = 3
    ...
    /// Transport gate (v1-architecture.md §8): decides, from each snapshot
    /// refresh's `capabilities.ws_transport_enabled`, whether the
    /// RealtimeClient may connect. Pure decision logic lives in
    ...
    private func startRealtime() {
        // The transport gate (v1-architecture.md §8): only ever connect once
        // a snapshot refresh has confirmed `capabilities.ws_transport_enabled
        // == true`. Everything else in this method is unchanged pre-existing
        // plumbing.
    ...
    /// Apply the capability bit from a fresh snapshot refresh via the pure
    /// gate, then act on whatever it decides: false→true may open a
    /// connection (this refresh IS the "a refresh says true again" the gate
    /// requires before reconnecting); true→false (server rolled the flag
    /// back, or `WS_TRANSPORT_ENABLED` is off) tears down any live
    /// connection and falls back to plain HTTP polling of the same
    /// `/v1/snapshot` — transport degradation only, never a different data
    /// source (v1-architecture.md §0, §8).
    ...
    private func applyRealtimeState(_ state: SpaceState) {
        ...
        tasks = state.uiTasks
        // This device's local display overrides layer on top of the
        // delta-derived metric state — v1 has no server-side metric display
        // preference route (v1-architecture.md §2.3, §13.2).
        let sortedMetrics = state.uiMetrics.map(MetricPreferencesStore.apply(to:))
    ...
    private func handleRealtimeNotice(_ notice: RealtimeClient.Notice) {
        ...
        case .transportUnavailable:
            // A 503'd WS upgrade (v1-architecture.md §8.1): this is a
            // capability fact, not a network hiccup — reset the gate so a
            // stale cached `true` doesn't mask the next snapshot's real
            // answer, and fall back to HTTP immediately rather than waiting
            // out the ordinary backoff cycle count.
            realtimeGate.reset()
            startFallbackPolling()
        }
    }
    ...
    /// v1 bumps the credential format from `st2_` to `sr1_`
    /// (v1-architecture.md §10.3) and moves the whole REST surface from
    /// `/v2`+`/v3` to `/v1` with no back-compat parsing on either axis.
    /// Keychain values survive an app uninstall, so clear them once instead
    /// of silently retrying a stale token/server pairing against the new
    /// API forever — the device just re-pairs via the ordinary invite flow.
    ...
    /// v1 has no `PATCH /v1/metrics/:id` (v1-architecture.md §2.3, §13.2:
    /// deferred to v1.1 — cross-viewer preference sync needs its own
    /// convergence channel). Display overrides are entirely client-local:
    /// no network round trip, applied immediately. `level` is accepted for
    /// call-site compatibility but was never part of any folded metric
    /// shape (pre-existing, unrelated to this port).
    func updateMetric(id: String, icon: String? = nil, tint: String? = nil,
        ...
        MetricPreferencesStore.update(
            id, icon: icon, tint: tint, template: template,
            alertAbove: alertAbove, alertBelow: alertBelow)
        metrics = metrics.map { $0.key == id ? MetricPreferencesStore.apply(to: $0) : $0 }
        WidgetCenter.shared.reloadAllTimelines()
    }
    ...
    func refresh() async {
        ...
        do {
            ...
            lastError = nil
            lastSyncAt = .now
            applyRealtimeCapability(snapshot.capabilities.wsTransportEnabled)
            ...
            // This device's local display overrides layer on top of the
            // server-reported metric state — v1 has no server-side metric
            // display preference route (v1-architecture.md §2.3, §13.2).
            let sorted = snapshot.metrics.map { MetricPreferencesStore.apply(to: $0.state) }
                .sorted { $0.key < $1.key }
            // Widgets refresh on a slow OS budget; while the app is foreground
