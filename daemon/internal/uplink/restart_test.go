...
func TestOverflowSurvivesProcessRestartAndDeliversInOrder(t *testing.T) {
	...
	// delivery — the HTTP /v1/events spy must never have carried either
	// reliable event: the realtime Client owns durable delivery, so a fork
	// onto the non-realtime uplink here would be a silent, permanent miss.
	if msgs := httpCap.byType(wire.TypeMessageEvent); len(msgs) != 0 {
		t.Fatalf("HTTP /v1/events carried %d message.event(s); reliable events must travel the realtime path only, restart or not", len(msgs))
	}
}
