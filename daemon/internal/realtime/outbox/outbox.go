...
func openWithBusyTimeoutMS(path string, maxRows, busyTimeoutMS int) (*Store, error) {
	...
	store := &Store{db: db, maxRows: maxRows}
...
func (s *Store) Enqueue(ctx context.Context, space, kind string, build BuildBody) (Item, error) {
	...
	for attempt := 0; attempt < enqueueMaxAttempts; attempt++ {
		...
		item, terminal, err := s.enqueueAttempt(ctx, space, kind, build)
		if err == nil {
			return item, nil
		}
		if terminal {
			// ErrOverflowed (success-shaped) or a BuildBody/validation
			// failure (the same input can never validate differently):
			// neither benefits from a retry.
			return item, err
		}
		lastErr = err
	}
...
// non-nil (a BuildBody failure, or the informational ErrOverflowed).
func (s *Store) enqueueAttempt(ctx context.Context, space, kind string, build BuildBody) (item Item, terminal bool, err error) {
	...
	if overflowPending > 0 || rows >= s.maxRows {
		if err := s.insertOverflowTx(ctx, tx, space, kind, build); err != nil {
...
// insertOverflowTx persists the pre-seq event material into outbox_overflow
// within tx. build is called with a placeholder device_seq of 1 — the
// smallest value wire.Validate accepts — purely so validation can run; the
// real device_seq is patched in at promotion (see setDeviceSeq) and never
// observed on the wire in its placeholder form.
func (s *Store) insertOverflowTx(ctx context.Context, tx *sql.Tx, space, kind string, build BuildBody) error {
	const placeholderSeq = 1
	body, err := build(placeholderSeq)
	if err != nil {
		return fmt.Errorf("outbox: build overflow body: %w", err)
	}
	now := time.Now().UnixMilli()
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO outbox_overflow (space, kind, body, created_at) VALUES (?, ?, ?, ?)`,
		space, kind, string(body), now,
	); err != nil {
		return fmt.Errorf("outbox: insert overflow: %w", err)
	}
	return nil
}
...
// failure here is logged-and-swallowed rather than failing the ack itself
// (the ack succeeded; the event really was acknowledged) — the next ack or
// the caller's own periodic tick gets another chance.
func (s *Store) Ack(ctx context.Context, space string, seq int64) error {
	...
	_, _ = s.PromoteOverflow(ctx, 1) //nolint:errcheck // best-effort; see doc comment
	return nil
}
