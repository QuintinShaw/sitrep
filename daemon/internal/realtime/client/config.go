...
//     outbox before attempting delivery, retire it on a matching ack
//     (WS) or acked[] pair (HTTP), and replay every still-unacked event
//     oldest-first after each reconnect.
//   - Degrade transport, never store: per
//     docs/design/v1-architecture.md section 0, there is exactly ONE
//     SpaceHub this client's outbox ever targets. When the WebSocket
//     transport is unavailable — the socket is down/reconnecting, or the
//     server answered the upgrade with 503 transport_unavailable
//     (WS_TRANSPORT_ENABLED off) — the SAME reliable events and metric
//     frames are sent over HTTP POST /v1/events to the SAME space instead,
//     using the shared ACK shape (section 4.1) to retire outbox rows with
//     the identical resend-queue logic the WS ack path uses. See
//     httpFallbackLoop/flushHTTP and the flushMu single-writer discipline
...
type Config struct {
	// URL is the ws:// or wss:// endpoint to dial (GET /v1/realtime).
	URL string
	// EventsURL is the https:// or http:// endpoint for the HTTP transport
	// fallback (POST /v1/events, docs/design/v1-architecture.md section
	// 4). Empty disables the HTTP fallback entirely: the client behaves
	// exactly as if WS were the only transport (useful for tests that
	// don't exercise it, and for a caller that has no HTTP endpoint to
	// give). In production this should always be set alongside URL —
	// both resolve to the same SpaceHub instance for the same space.
	EventsURL string
	// Token is presented once at connection establishment (SPEC.md section
	// 10) as an Authorization: Bearer header for WS, and on every request
	// for HTTP — never repeated inside an envelope either way.
	Token string
	...
	// suspenders on top of the mandatory post-reconnect replay). It is
	// also the HTTP fallback loop's poll/flush cadence.
	...
	// HTTPTimeout bounds one POST /v1/events request (dial+response).
	...
	// OnAuthState surfaces persistent authentication failures (an HTTP 401 —
	// e.g. this device's token was revoked) so the daemon can route them to
	// the health-status file the menubar reads, instead of failing silently.
	// It is called only on a TRANSITION: (false, reason) the first time a
	// 401 is seen at WS dial or on the HTTP /v1/events flush, and (true, "")
	// once a request authenticates again. nil is a no-op.
	OnAuthState func(ok bool, reason string)

	// testWSReprobeInterval overrides wsReprobeInterval, the fixed wait
	// before re-attempting a WS dial after it was rejected with
	// ErrTransportUnavailable (503). Unexported: there is no public config
	// surface for this (a deployment's WS_TRANSPORT_ENABLED flag is not
	// something a user should be tuning client-side). Set only via
	// WithTestWSReprobeInterval, and only from tests.
	testWSReprobeInterval time.Duration
}

// WithTestWSReprobeInterval returns a copy of cfg with the periodic WS
// re-probe interval (wsReprobeInterval, normally ~5 minutes) shortened.
// This is a test-only seam for integration tests that need to observe a
// 503-rejected -> re-enabled WS transition without waiting 5 real minutes;
// production code must never call this.
func WithTestWSReprobeInterval(cfg Config, d time.Duration) Config {
	cfg.testWSReprobeInterval = d
	return cfg
}
