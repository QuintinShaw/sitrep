...
func newTestOutboxWithMaxRows(t *testing.T, maxRows int) *outbox.Store {
	t.Helper()
	store, err := outbox.OpenWithMaxRows(filepath.Join(t.TempDir(), "outbox.db"), maxRows)
	if err != nil {
		t.Fatalf("outbox.OpenWithMaxRows: %v", err)
	}
	t.Cleanup(func() { store.Close() })
	return store
}
