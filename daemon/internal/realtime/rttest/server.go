// Package rttest is a test-only, minimal WebSocket server used to exercise
// the realtime client (internal/realtime/client) against scripted server
// behavior: hello accept, acking, withholding acks, dropping connections,
// sending commands, and the ping/pong heartbeat.
//
...
// SPEC.md describes without a network dependency on a real server. It must
// never be described as, or grow into, a self-hosted server implementation.
package rttest
...
// Server is a scriptable mock realtime endpoint bound to an httptest server.
type Server struct {
